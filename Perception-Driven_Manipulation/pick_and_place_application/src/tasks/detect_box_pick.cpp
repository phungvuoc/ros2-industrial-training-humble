...
geometry_msgs::msg::Pose pick_and_place_application::PickAndPlaceApp::detectBox()
{
  ...
  auto response_fut = target_recognition_client->async_send_request(req);
  ...
  if (st == std::future_status ::ready)
  {
    ...
    if (response->succeeded)
    {
      ...
      RCLCPP_INFO_STREAM(node->get_logger(), "\033[1;32mtarget recognition succeeded\033[0m");
    }
    else
    {
      RCLCPP_ERROR_STREAM(node->get_logger(), "\033[1;31mTarget recognition failed\033[0m");
