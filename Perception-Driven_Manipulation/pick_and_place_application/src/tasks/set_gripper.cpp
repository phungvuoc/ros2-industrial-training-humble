...
void pick_and_place_application::PickAndPlaceApp::actuateGripper(bool do_grasp)
{
  ...
  {
    if (do_grasp)
      RCLCPP_INFO_STREAM(node->get_logger(), "\033[1;32mGripper closed\033[0m");
    else
      RCLCPP_INFO_STREAM(node->get_logger(), "\033[1;32mGripper opened\033[0m");
  }
  else
  {
    RCLCPP_ERROR_STREAM(node->get_logger(), "\033[1;31mGripper failure\033[0m");
    exit(1);
  }
}
