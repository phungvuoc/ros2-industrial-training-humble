...
void pick_and_place_application::PickAndPlaceApp::moveToWaitPosition()
{
  ...
  if (plan_solution)
  {
    RCLCPP_INFO_STREAM(node->get_logger(), "\033[1;32m" << "Move " << cfg.WAIT_POSE_NAME << " Succeeded" << "\033[0m");
  }
  else
  {
    RCLCPP_INFO_STREAM(node->get_logger(), "\033[1;31m" << "Move " << cfg.WAIT_POSE_NAME << " Failed" << "\033[0m");
